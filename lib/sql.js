    ...
    var compareModel = function(oldModelDef, newModelDef) {
        ...
        if (oldColumnDef) {
          if (!_.isEqual(oldColumnDef, columnDef)) {
            if (oldColumnDef.columnName != columnDef.columnName)
              script.push(agent.renameColumn(tableName, oldColumnDef.columnName, columnDef.columnName));
          ...
          if (similarColName) {
            ...
            script.push(agent.renameColumn(tableName, similarCol.columnName, columnDef.columnName));
      ...
      if (!_.isEqual(oldModelDef.primaryKey, newModelDef.primaryKey)) {
        if (!oldModelDef.primaryKey && newModelDef.primaryKey)
          deferedScript.push(agent.createPrimaryKeys(tableName, newModelDef.primaryKey));
          ...
          script.push(agent.dropPrimaryKey(tableName, oldModelDef.primaryKey));
                 ...
                 !isColumnsEqual(oldModelDef.primaryKey.columns, newModelDef.primaryKey.columns)) {
          script.push(agent.dropPrimaryKey(tableName, oldModelDef.primaryKey));
          deferedScript.push(agent.createPrimaryKeys(tableName, newModelDef.primaryKey));
        }
        /*
      ...
      _.forEach(oldModelDef.indexes, function(oldIndexInfo) {
        ...
        if (newIndexInfo) {
          if (newIndexInfo.name != oldIndexInfo.name)
            script.push(agent.renameIndex(oldIndexInfo.name, newIndexInfo.name));
