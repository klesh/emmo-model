...
function Migrator(em) {
  ...
  var migrations = fs.readdirSync(this.migrationsPath).filter(function(f) {
    ...
    var d = {
      ...
      name: m[1],
      fileName: f,
      script: fs.readFileSync(path.resolve(migrationsPath, f)).toString().trim()
    };
    ...
    if (fs.existsSync(js)) { // come with a customize .js file to execute the migration
      var part1 = d.script.split('/***** PLACE YOUR CUSTOMIZE SCRIPT HERE *****/');
      if (part1.length === 2) {
        d.scriptBefore = part1[0].trim();
        var part2 = part1[1].split('/******** END YOUR CUSTOMIZE SCRIPT *********/');
        if (part2.length === 2) {
          d.scriptCustomize = part2[0].trim();
          d.scriptAfter = part2[1].trim();
        }
      }
  ...
  this.migrations = migrations.sort((a, b) => a.uid - b.uid);
}
...
Migrator.prototype.getCreateMigrationSQL = function() {
  if (!this.createMigrationScript)
    this.createMigrationScript = Sql.initialScript(this.dialect, {'_Migration': this.purified['_Migration']});
...
Migrator.prototype.lastMigrationData = function() {
  ...
  if (this.migrations && this.migrations.length) {
    ...
    data = { uid: last.uid, name: last.name }; // accept only newer than last migration
  } else {
