...
Session.prototype.update = function(modelName, data, where) {
  ...
  return this.query(sql, values).then(rows => rows.affectedRows);
};
...
/** shortcut method, automatically validate then insert or update base on AutoIncrement property value
 *  pass forceUpdate a true value if model doesn't have a AutoIncrement column.
 *
 *  insert: db.save('User', { nick: 'foo' });
 *  update: db.save('User', { nick: 'bar', id: 1 });
 */
Session.prototype.save = function(modelName, data, forceUpdate) {
  var Model = this.em.models[modelName], self = this, modelDef = this.em.definition[modelName];
  var isUpdate = forceUpdate || data[modelDef.autoIncrementColumnName] > 0;
  return Model.validate(data, isUpdate).then(function() {
    return self[ isUpdate ? 'update' : 'insert' ](modelName, data);
  });
};

/** shortcut method, to update a cell value, useful when applying EDIT IN PLACE 
 *  
 */
Session.prototype.cell = function(modelName, field, value, pkvalue) {
  var Model = this.em.models[modelName], self = this, modelDef = this.em.definition[modelName];
  var pks = modelDef.primaryKeys;
  ...
  return Model.validateValue(field, value).then(function() {
    var params = {};
    params[pks[0]] = pkvalue;
    params[field] = value;
    return self.update(modelName, params);
  });
};
...
Session.prototype.select = function(modelName, options) {
  ...
  function quoteField(f) {
    ...
    if (_.isPlainObject(f)) {
      return _.map(f, function(f2, alias) {
        return quoteField(f2) + ' AS ' + alias;
      }).join(', ');
    }
    if (_.isArray(f)) {
      return _.map(f, quoteField).join(', ');
    }
  ...
  if (options.field) {
    field = quoteField(options.field);
    //if (_.isArray(options.field))
      //field = _.intersection(options.field, modelDef.columnNames).map(quoteField).join(', ');
    //else
