 ...
 */
Session.prototype.insert = function(entityName, data) {
  var b = new Builder(this.em, entityName).insert(data);

  var self = this;
  return this.query(b.sql, b.values).then(function(rows) {
    if (b.entity.autoIncrementName) {
      return b.agent.returnId(self.connection).then(function(id) {
 ...
 * @type {object}
 * @property {string}     column    timestamp column's name
 * @property {string[]}   keys      compare only those keys
 */

/**
 * set update-timestamp column's value to now if data is different from database, 
 ...
 * @returns {Promise}
 */
Session.prototype.timestamp = function(entityName, data, options) {
  var column = options && options.column ? options.column : 'updatedAt';
  var keys = options && options.keys ? options.keys : _.keys(data);
  var em = this.em;
  
  if (data.hasOwnProperty(column)) data = _.omit(data, column);
  var condition = _.pick(data, _.union(keys, em.entities[entityName].primaryKeyNames));
  
  return this.count(entityName, condition).then(function(count) {
    if (count === 0) {
      data[column] = em.now(); 
      return true;
    }
 ...
 * User.refresh({ id: 10, firstName: 'new guy' });
 *
 * // or you have a different columnName(modifiedAt):
 * User.refresh({ id: 10, firstName: 'new guy' }, { column: 'modifiedAt' })
 *
 * // or just compare only certain properties, this would not update:
 * User.refresh({ id: 10, remark: 'new remark' }, { keys: [ 'firstName', 'lastName', 'age' ] })
 * 
 *
 * @param {string}              entityName
 * @param {object}              data
 ...
 * @returns {Promise}
 */
Session.prototype.refresh = function(entityName, data, options) {
  var self = this;
  return this.timestamp(entityName, data, options).then(function(changed) {
    if (changed === true || (options && options.keys)) // partial comparison may still need to run a update.
      return self.update(entityName, data);
  });
};
