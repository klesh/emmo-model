...
Session.prototype.update = function(modelName, data, where) {
  ...
  var columns = _.keys(updatable);
  var values = _.values(updatable);
  where = where ? _.pick(where, modelDef.columnNames) : _.pick(data, modelDef.primaryKeys.columns);
  var index = 0;
...
Session.prototype.select = function(modelName, options) {
  ...
  if (options.where) {
    ...
    _.forOwn(_.pick(options.where, modelDef.columnNames), function(v, k) {
      values.push(v);
      sql.push(util.format("%s=%s", agent.quote(k), agent.placehold(values.length - 1)));
    });
  }
  if (options.order) {
    sql.push('ORDER BY');
