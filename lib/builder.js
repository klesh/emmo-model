...
Builder.prototype.insert = function(data, options) {
  ...
  var keys = _.intersection(_.keys(data), propertyNames);
...
Builder.prototype.update = function(data, where) {
  ...
  if (_.isEmpty(data)) // nothing to update
    return;
...
Builder.prototype.select = function(options) {
  ...
  */
  ...
  if (options.groupby) {
    var groupby = _.isArray(options.groupby) ? options.groupby : [options.groupby];
    b.append('GROUP BY').append(_.map(groupby, f => this.quote(f)));
