...
function createModel(em, modelName, modelDef) {
  ...
  Model.find = function() {
    ...
    if (!_.isObject(params)) { // to support find by primary keys, like find(1), find(1, 2) 
      var pks = modelDef.primaryKeys;
      // find(1, 3) for multiple primary keys situation
      if (arguments.length === pks.length) {
        params = {};
        for (var i = 0, j = arguments.length; i < j; i++) {
          params[pks[i]] = arguments[i];
        }
      }
    }
    return em.scope(db => db.find(modelName, params));
  };
