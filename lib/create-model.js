...
function createModel(em, modelName, modelDef) {
  ...
  // validate instance
  Model.validate = function(data, propertyNames) {
    if (propertyNames === true || data[modelDef.autoIncrementColumnName] > 0) { // validate only inst keys ( as in update mode )
      propertyNames = _.intersection(modelDef.columnNames, _.keys(data));
    ...
    return Promise2.each(propertyNames, function(propertyName) {
      return Model.validateProperty(data, propertyName);
    });
  };

  // convert property value to appropriate type and validate
  Model.validateProperty = function(data, propertyName) {
  ...
  _.each(_.keys(Session.prototype), function(methodName) {
